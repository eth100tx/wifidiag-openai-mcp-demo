...
class MCPChatGPTClient:
    def __init__(self):
        ...
        self.endpoint = "https:/dev-openai-eastern..."
        self.model_name = "gpt_4o"
        self.deployment = "gep-4o-special"
        self.subscription_key = "API KEY"
    ...
    def initialize_openai(self, api_key: str):
        ...
        try:
            self.status_queue.put("Connecting to Azure OpenAI API...")
            ...
            self.status_queue.put("Azure OpenAI API connected successfully")
            return True
        except Exception as e:
            error_msg = f"Azure OpenAI API connection failed: {str(e)}"
            self.status_queue.put(error_msg)
            return False
    ...
    async def chat_with_gpt(self, message: str):
        ...
        try:
            ...
            # Make the API call using Azure deployment
    ...
    def send_message_sync(self):
        ...
        if not self.openai_client:
            self.display_message("Azure OpenAI API not connected. Please check your API key.", "error")
            return
    ...
    def setup_gui(self):
        """Setup the GUI"""
        self.root = tk.Tk()
        self.root.title("MCP + ChatGPT Azure Demo Client")
        ...
        # Welcome message
        welcome_msg = """Welcome to MCP + ChatGPT Azure Demo Client!

This client connects your MCP server to ChatGPT Azure, allowing ChatGPT to use your MCP tools.

Setup required:
1. Set OPENAI_API_KEY environment variable or .env file (Azure subscription key)
        ...
        try:
            ...
            if not api_key:
                api_key = simpledialog.askstring(
                    ...
                    "Enter your Azure OpenAI API Key (subscription key):",
                    show='*'
                )
                if not api_key:
                    messagebox.showerror("Error", "Azure OpenAI API key is required")
                    return
            ...
            def init_openai():
                if self.initialize_openai(api_key):
                    ...
                    self.status_queue.put("Failed to connect to Azure OpenAI")
